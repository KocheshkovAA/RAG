...
### CoT — внутренняя декомпозиция

Реализовано в двух местах, оба — программная декомпозиция без утечки сырого chain-of-thought наружу:

- `QueryOptimizer` (`app/core/query_processor.py`, используется в `RAG.get_relevant_documents()`
  за `QUERY_OPTIMIZER_ENABLED`) — одноходовая декомпозиция сложного вопроса на 1-3 под-запроса
  перед retrieval, без обратной связи по результату.
- `RouteDecision.reasoning` (`orchestrator.py`) — структурированное поле обоснования маршрута,
  используется только для трассировки (Langfuse), не показывается пользователю.

Наружу (в API-ответ) CoT никогда не попадает как текст — только как побочный эффект (набор
retrieval-запросов) или debug-метаданные в трейсинге.
