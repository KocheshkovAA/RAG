...
AGENTIC_SYSTEM_PROMPT = (
    ...
    "Правила:\n"
    "- Перед КАЖДЫМ ходом (вызовом инструмента или остановкой) кратко, одним "
    "предложением, напиши в тексте ответа мысль: что собираешься искать и "
    "почему, либо почему данных уже достаточно. Это обязательная часть хода, "
    "не пропускай её даже при вызове инструмента.\n"
    ...
    "останови вызовы инструмента — просто ответь мыслью из первого правила, "
    "без вызова инструмента.\n"
    "- Твой текстовый ответ в этом чате НЕ показывается пользователю и не "
    "используется как финальный ответ — тот строится отдельно из найденного "
    "контекста. Но мысль записывается в лог хода (для отладки/трассировки), "
    "так что пиши по существу, а не заглушку."
)
    ...
    async def answer(self, question: str, usage_handler=None, include_debug_docs: bool = False):
        ...
        with propagate_attributes(tags=["rag", "warhammer", "agentic"]):
            ...
            for round_idx in range(self.max_iterations):
                ...
                # CoT-часть ReAct-хода: короткая мысль модели про следующий шаг,
                # запрошенная явно в AGENTIC_SYSTEM_PROMPT. Не показывается
                # пользователю (см. finalize ниже), но фиксируется в agentic_meta
                # для отладки/трассировки — иначе это неотличимо от голого
                # вызова инструмента без всякого рассуждения.
                thought = (ai_msg.content or "").strip()
                if thought:
                    thoughts.append({"round": round_idx + 1, "thought": thought})
