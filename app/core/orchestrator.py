...
class WarhammerOrchestrator:
    def __init__(self, vector_rag: RAG, light_rag: LightRAGClient, agentic_rag: AgenticRAG):
        ...
        if settings.AGENTIC_ROUTE_ENABLED:
            agentic_instruction = (
                "\nЕсть также 'agentic' (агентный поиск: модель сама решает, сколько раз и с какими "
                "запросами обратиться к базе знаний, включая параллельные запросы на разные сущности "
                "за один ход) — выбирай его для составных фактических вопросов, где обычный единичный "
                "поиск может не найти всё сразу (несколько разных сущностей/дат/названий в одном "
                "вопросе), но это НЕ вопрос про Ересь Хоруса с анализом причин и связей (тот остаётся "
                "'graph'). При сомнении между 'agentic' и 'vector' — выбирай 'vector'.\n"
            )
