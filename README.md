...
- **Промпт-роутер даёт ложные срабатывания** на реляционные формулировки за пределами охвата графа (пункт 5 выше, refusal_rate 0.35 на capstone-прогоне) — LoRA-роутер (96.3% held-out accuracy) уже обучен и потенциально решает эту проблему точности, но не задеплоен.
- **`AnswerFaithfulnessGuard` не гоняется по финальному ответу graph-маршрута** — только по vector-side domain probe и грубой эвристике «пустого» ответа LightRAG. Чем больше синтеза нужно для сборки ответа (multi-hop по определению), тем выше риск незамеченной галлюцинации.
