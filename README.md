# RAG Telegram Bot с графовой базой знаний и ИИ-агентом
Проект представляет собой Telegram-бота, который отвечает на вопросы о вселенной Warhammer 40k с использованием Retrieval-Augmented Generation (RAG). 
Бот комбинирует поиск по векторному хранилищу документов и графовый анализ для генерации информативных ответов. 
Графовая база Neo4j используется для фильтрации нерелевантных фрагментов, поиска дополнительных релевантных чанков и обогащения контекста за счёт связей между фрагментами.

### **Ключевые компоненты и возможности**
* **Создание и подготовка собственного датасета** из статей Warhammer 40k.
* **Fine-tune модели эмбеддингов** на этом датасете для повышения точности поиска.
* Разделение сложных вопросов на под-вопросы и извлечение ключевых сущностей с помощью **LLM**.
* **ИИ-агент**: Использование **LangGraph** для создания циклического агента-оптимизатора, который самостоятельно принимает решение: достаточно ли данных в контексте, нужно ли удалить «мусорные» узлы или расширить поиск через графовые связи.
* Трассировка и мониторинг: Интеграция с **LangSmith** для полной видимости цепочек рассуждений, отладки инструментов агента и анализа стоимости токенов.
* Поиск релевантных документов в **Chroma-векторном хранилище** по под-вопросам и сущностям.
* Фильтрация нерелевантных фрагментов и расширение контекста через связи сущностей в **графовой базе Neo4j**.
* Генерация информативного ответа с использованием **LLM**, обогащённого графовым контекстом.
* Отправка форматированных ответов пользователю через **Telegram**, включая ссылки на источники.
* Контейнеризация и деплой через **Docker**.

## 👉 Попробовать бота: [@WarhammerWikiBot](https://t.me/WarhammerWikiBot)

## 🚀 Запуск проекта

Проект полностью контейнеризован и готов к развёртыванию.  
Для запуска достаточно выполнить команду:

```bash
docker compose up --build
```

## **Пайплайн**

1. Пользователь отправляет вопрос в Telegram.
2. Вопрос разбивается на под-вопросы, извлекаются ключевые сущности.
3. Ретривер ищет релевантные документы в Chroma-векторном хранилище.
4. **Графовая база**:
   1. фильтрует нерелевантные фрагменты,
   2. ищет дополнительные релевантные узлы,
   3. добавляет контекст о взаимосвязях между чанками.
5. **Агентная оптимизация контекста (LangGraph):**
   1. Оценка: LLM-агент анализирует текущий набор узлов из векторного и графового поиска.
   2. Очистка (Pruning): Агент удаляет из контекста узлы, если они не относятся к сути вопроса.
   3. Доразведка (Expansion): Если данных не хватает, агент вызывает инструмент поиска связей в Neo4j, чтобы «подтянуть» недостающие факты.   
6. LLM генерирует финальный ответ с учётом найденного контекста.
7. Ответ форматируется и отправляется пользователю через Telegram.

<img width="1247" height="654" alt="RAG drawio(1)" src="https://github.com/user-attachments/assets/41b7fafb-d4db-46c8-930e-79192051eaab" />

### **Технологический стек**

* **Язык и фреймворки:** Python, aiogram — реализация Telegram-бота и логики взаимодействия с пользователем.
* **LLM:** GigaChat — генерация ответов, разбиение вопросов и извлечение сущностей.
* **RAG-оркестрация:** LangChain — управление пайплайном поиска и генерации.
* **Векторное хранилище:** Chroma — быстрый поиск релевантных чанков по эмбеддингам.
* **Agentic Framework:** LangGraph.
* **Monitoring & Tracing:** LangSmith.
* **Генерация эмбеддингов:** sentence-transformers + fine-tuning на кастомном датасете.
* **Графовая база знаний:** Neo4j — хранение сущностей и связей для фильтрации и расширения контекста.
* **База статей:** SQLite — хранение исходных текстов, чанков и сгенерированных вопросов.
* **Инфраструктура:** Docker, docker-compose — контейнеризация и деплой.

### **Источник и подготовка данных:**
* Данные собирались с [Warhammer 40k Fandom](https://warhammer40k.fandom.com/ru/wiki/Warhammer_40000_Wiki) с помощью парсера на Python.
* Статьи сохраняются в **SQLite базу**.
* Из текстов извлекаются источники и ключевые ссылки между сущностями для последующего использования в графовой базе.
* Тексты статей делятся на семантически осмысленные фрагменты (чанки).

## **Дообучение (fine-tuning) модели эмбеддингов**
* Для каждого чанка с помощью LLM (**GigaChat**) генерировались 4 вопроса, покрывающих факты и объяснения из фрагмента, в формате JSON.
* Результаты сохранялись в базе данных SQLite.
* В качестве базовой модели использовалась **sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2**.
* Дообучение проводилось на сгенерированных парах «чанк — вопрос».
* После дообучения эмбеддинги интегрировались в **Chroma**, что повышало точность поиска и качество генерации в RAG-пайплайне.

## **Формирование графовой базы знаний**

Графовая база знаний в Neo4j строится на основе данных из статей. Каждый элемент графа несёт позволяет моделировать взаимосвязи между сущностями.
#### **Элементы графа**
1. **Узлы (Nodes)**  
   Узлы представляют ключевые сущности и объекты вселенной, например: персонажи, организации и сражения.
   Каждый узел содержит метаданные: название, описание и категории. 
2. **Связи (Relationships)**  
   Связи отражают отношения между сущностями:  
   - Прямые действия и принадлежность (`КОМАНДОВАНИЕ`, `УЧАСТВУЕТ`, `ПРЕДСТАВЛЯЕТ`).  
   - Исторические и логические связи (`ПРЕДЫДУЩАЯ`, `СЛЕДУЮЩАЯ`).  
   - Ссылки на источники и статьи (`ССЫЛКА`).  

#### **Процесс построения графа**
1. Для каждой статьи (сущности) создаётся узел с метаданными и типами.  
2. Формируются связи между узлами на основе инфобоксов и ссылок между статьями. 

В результате формируется структурированная графовая база, в которой узлы отражают ключевые сущности, а связи моделируют их взаимодействия и принадлежности.  

## **Использование графовой базы знаний для обогащения контекста**

После извлечения релевантных документов с помощью векторного поиска, графовая база Neo4j используется для дополнительной фильтрации и расширения контекста:
1. **Фильтрация нерелевантных фрагментов**  
   Для найденных документов оценивается их связность в графе. Узлы с нулевым графовым весом и одиночные фрагменты могут быть исключены, что снижает шум и повышает точность ответа.
2. **Поиск дополнительных релевантных узлов**  
   На основе найденных узлов вычисляются кратчайшие пути между ними, и промежуточные узлы добавляются в контекст. Это позволяет учитывать связанные сущности, которые не были найдены напрямую в векторном поиске.
3. **Обогащение контекста связями**  
   Для каждого узла можно извлечь входящие и исходящие связи с другими сущностями, а также метаданные. Эта информация включается в итоговый текстовый контекст для LLM.
4. **Формирование графовых путей**  
   Дополнительно создаются отдельные текстовые блоки с описанием найденных путей между узлами, включая типы отношений.

Это помогает LLM учитывать взаимосвязи между сущностями при генерации ответа.

**Преимущества использования графа:**
- Улучшает качество поиска, объединяя векторный и структурный подходы.
- Позволяет находить скрытые связи между объектами.
- Снижает вероятность пропуска релевантной информации при генерации ответа.

## **Пример**
Пользователь задаёт вопрос:  
**«Как связаны Абаддон и Тразин?»**
1. Вопрос содержит несколько сущностей: *Абаддон* и *Тразин*.  
2. **Поиск в графе**  
   - Сначала векторный поиск извлекает статьи про Абаддона и Тразина.  
   - Затем выполняется поиск кратчайших путей в Neo4j между найденными узлами, исключая нерелевантные связи.  
   - В результате формируется путь, включающий промежуточные узлы и типы связей.
3. **Результат**  
   LLM получает не только описание отдельных персонажей, но и графовую структуру, показывающую их связь.  
   Например:  

<img width="951" height="654" alt="image" src="https://github.com/user-attachments/assets/f1c9005a-8fb4-40fc-9c88-6f65ed7504b4" />


## **Пример Трассировки**

<img width="834" height="396" alt="image" src="https://github.com/user-attachments/assets/1d2e05d2-e976-49b7-84f2-c4415b91ddbb" />


