...
> Граф построен только по одной статье ("Ересь Хоруса", ~830 сущностей) — сознательное решение, чтобы не жечь токены на индексацию всей вики. Демо-вопросы на graph-маршруте стоит держать в скоупе этой статьи (примархи, легионы, ключевые события Ереси) — за его пределами возможны ложные "не найдено".
...
> 60 вопросов — ориентировочная выборка для быстрой итерации на pet-проекте, не статистически значимый бенчмарк (индустриальный минимум для риск-валидации — порядка 250-500 вопросов). Цифры ниже стоит читать как направление, а не как строгую метрику.
